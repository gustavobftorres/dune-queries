...
-- query name: Balancer Volume by Source (Volume Breakdown)
-- query link: https://dune.com/queries/2944378
...
WITH 
    raw_swaps AS (
        ...
        AND ('{{6. Pool ID}}' = 'All' OR project_contract_address = CAST('{{6. Pool ID}}' AS varbinary))
        AND ('{{1. Source Address}}' = 'All' OR CAST(tx_to as VARCHAR) = '{{1. Source Address}}')
...
WHERE ('{{1. Source Address}}' = 'All' OR c.channel = '{{1. Source Address}}')
AND ('{{5. Source Name}}' = 'All' OR c.class = '{{5. Source Name}}')
GROUP BY 1,2
