...
-- query link: https://dune.com/queries/3954614
...
WITH volume AS (
    ...
    ,CONCAT('<a target="_blank" href="https://dune.com/balancer/balancer-cowswap-amm-pool?1.+Pool+Address_tc1e0a=', SUBSTRING(CAST("poolID" AS VARCHAR), 1, 66), '&4.+Blockchain_t4bd30=', q.blockchain, '">Stats ↗</a>') AS stats,
        CASE
            ...
            WHEN q.blockchain = 'gnosis' THEN CONCAT('<a target="_blank" href="https://balancer.fi/pools/base/cow/', CAST("poolID" AS VARCHAR), '">app ↗</a>')
END AS pool,
        CASE
            ...
            WHEN q.blockchain = 'gnosis' THEN CONCAT('<a target="_blank" href="https://basescan.com/address/0', SUBSTRING(CAST("poolID" AS VARCHAR), 2, 41), '">⛓</a>')         
 END AS scan
    ...
    WHERE
        ('{{2. Blockchain}}' = 'All' OR v.blockchain = '{{2. Blockchain}}')
        AND "TVL" IS NOT NULL
    ORDER BY 3 DESC
