...
-- query name: incremental vs view of bpt prices
-- query link: https://dune.com/queries/2641671
...
SELECT 'view' AS spell_type, * FROM output WHERE  hour >= date_trunc('hour', NOW() - INTERVAL '7' DAY )
UNION ALL
SELECT 'incremental' AS spell_type, * FROM balancer_v2_ethereum.bpt_prices WHERE  hour >= date_trunc('hour', NOW() - INTERVAL '7' DAY )
order by hour desc, 4