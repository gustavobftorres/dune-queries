...
-- query name: veBAL future balances
-- query link: https://dune.com/queries/3644589
...
WITH base_locks AS (
    ...
    cumulative_balances AS (
        SELECT
            day,
            provider,
            LEAD(DAY, 1, CAST(now() AS TIMESTAMP) + interval '1' year) OVER (
...
/*AND FROM_unixtime(unlocked_at) > NOW()
AND FROM_unixtime(unlocked_at) < NOW() + interval '30' day*/