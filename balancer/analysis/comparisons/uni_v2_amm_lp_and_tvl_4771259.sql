...
-- query link: https://dune.com/queries/4771259
...
pool as (
    select
        ...
        token0,
        token1
    from uniswap_v2_{{blockchain}}.UniswapV2Factory_evt_PairCreated
    where pair = {{pool}}
),
...
latest_syncs_per_day as (
    ...
    from uniswap_v2_{{blockchain}}.UniswapV2Pair_evt_Sync as syncs
...
prices AS(
    SELECT 
        DATE_TRUNC('day',minute) AS timestamp,
        contract_address,
        decimals,
...
tvl as (
    ...
    left join prices
        on
            ...
            balances.token = prices.contract_address
