...
-- query name: balancer_v1_trades (DuneSQL)
-- query link: https://dune.com/queries/2882120
...
WITH v1 AS (
    ...
    --{% if not is_incremental() %}
        where evt_block_time >= TIMESTAMP '2020-03-13'
    --{% endif %}
    --{% if is_incremental() %}
...
prices AS (
    ...
    --{% if not is_incremental() %}
        --and minute >= TIMESTAMP '2020-03-13'
    --{% endif %}
    --{% if is_incremental() %}
...
INNER JOIN ethereum.transactions tx
    ...
    --{% if not is_incremental() %}
    AND tx.block_time >= TIMESTAMP '2020-03-13'
    --{% endif %}
    --{% if is_incremental() %}
