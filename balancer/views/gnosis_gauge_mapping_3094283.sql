...
-- query name: gnosis gauge mapping
-- query link: https://dune.com/queries/3094283
...
WITH 
    root_gauges AS ( 
        ...
        FROM 
            (
                SELECT * FROM balancer_ethereum.GnosisRootGaugeFactory_evt_GaugeCreated
                CROSS JOIN (SELECT count(*) AS evt_record_count FROM balancer_ethereum.GnosisRootGaugeFactory_evt_GaugeCreated)
            ) x
        INNER JOIN 
            (
                SELECT * FROM balancer_ethereum.GnosisRootGaugeFactory_call_create
                CROSS JOIN (SELECT count(*) AS call_record_count FROM balancer_ethereum.GnosisRootGaugeFactory_call_create WHERE call_success = True) 
    ...
    , rg_kill_unkill AS (
        ...
        FROM balancer_ethereum.GnosisRootGauge_call_killGauge x 
        LEFT JOIN 
            (
                SELECT * FROM balancer_ethereum.GnosisRootGauge_call_unkillGauge 
                WHERE call_success = True
            ) y
        ...
        INNER JOIN 
            (
                SELECT gauge FROM balancer_ethereum.GnosisRootGaugeFactory_evt_GaugeCreated
            ) z
    ...
    , child_chain_gauges AS (
        ...
        FROM 
            (
                SELECT * FROM balancer_gnosis.ChildChainGaugeFactory_evt_GaugeCreated
                CROSS JOIN (SELECT count(*) AS evt_record_count FROM balancer_gnosis.ChildChainGaugeFactory_evt_GaugeCreated)
            ) x
        INNER JOIN 
            (
                SELECT * FROM balancer_gnosis.ChildChainGaugeFactory_call_create
                CROSS JOIN (SELECT count(*) AS call_record_count FROM balancer_gnosis.ChildChainGaugeFactory_call_create WHERE call_success = True) 
                WHERE call_success = True
            ) y
        ON y.output_0 = x.gauge
        AND y.call_record_count = x.evt_record_count -- Test .. No results return if False
        ...
        LEFT JOIN 
            (
                SELECT * FROM balancer_v2_gnosis.Vault_evt_PoolRegistered
            ) z
    ...
    , ccg_kill_unkill AS (
        ...
        FROM balancer_gnosis.ChildChainGauge_call_killGauge x 
        LEFT JOIN 
            (
                SELECT * FROM balancer_gnosis.ChildChainGauge_call_unkillGauge 
                WHERE call_success = True
            ) y
        ON y.contract_address = x.contract_address
        -- Possible to cause issue if kill/unkill happens in same block. See query 3093521
        ...
        INNER JOIN 
            (
                SELECT gauge FROM balancer_gnosis.ChildChainGaugeFactory_evt_GaugeCreated
            ) z 
        ON z.gauge = x.contract_address
        WHERE x.call_success = True
    )
