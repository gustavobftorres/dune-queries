...
-- query name: Optimism query after compile
-- query link: https://dune.com/queries/6521379
...
WITH pool_labels AS (
        SELECT * FROM (
            ...
            FROM "delta_prod"."labels"."balancer_v2_pools_optimism"
            WHERE blockchain = 'optimism'
            AND source = 'query'
            AND model_name = 'balancer_v2_pools_optimism'
            GROUP BY 1, 2, 3) 
        WHERE num = 1
    ),
    ...
    daily_protocol_fee_collected AS (
        ...
        FROM "delta_prod"."balancer_v2_optimism"."Vault_evt_FlashLoan" b
        GROUP BY 1, 2, 3 
        ...
        FROM "delta_prod"."balancer_v2_optimism"."Vault_evt_PoolBalanceChanged" b
        ...
        FROM "delta_prod"."balancer_v2_optimism"."Vault_evt_PoolRegistered" b
        INNER JOIN "delta_prod"."erc20_optimism"."evt_Transfer" t
            ON t.contract_address = b.poolAddress
            AND t."from" = 0x0000000000000000000000000000000000000000
            AND t."to" =
                CASE
                    WHEN 'optimism' = 'fantom' THEN 0xc6920d3a369e7c8bd1a22dbe385e11d1f7af948f
