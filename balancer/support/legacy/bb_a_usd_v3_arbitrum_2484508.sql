...
-- query name: bb-a-USD v3 (ARBITRUM)
-- query link: https://dune.com/queries/2484508
...
-----------------------------------------------------------------------------------
    ...
    pool_id_output AS (
        WITH 
            input AS (SELECT array[0x7c82a23b4c48d796dee36a9ca215b641c6a8709d000000000000000000000406, 0x9e34631547adcf2f8cefa0f5f223955c7b137571000000000000000000000407, 0x4739e50b59b552d490d3fdc60d200977a38510c0000000000000000000000409] AS pool_ids)
...
-----------------------------------------------------------------------------------
 ...
 -----------------------------------------------------------------------------------  
    ...
    pool_params AS (SELECT * FROM query_2407001 WHERE blockchain = 'arbitrum' AND linear_pool in (TABLE linear_pool_addresses)), -- ############# BLOCKCHAIN ################## !!
    -- Create a data subset where we get swaps from the Vault filtered by the inputted pool id(s)
    subset as (SELECT evt_block_time, poolId, tokenIn, tokenOut, amountIn, amountOut FROM balancer_v2_arbitrum.Vault_evt_Swap WHERE poolId in (TABLE pool_id_output)), -- ############# BLOCKCHAIN ################## !!
    -- Create a data subset where we get Balance changes by the pool manager from the Vault filtered by the inputted pool id(s)
    subset_2 as (SELECT evt_block_time, poolId, token, cashDelta, managedDelta FROM balancer_v2_arbitrum.Vault_evt_PoolBalanceManaged WHERE poolId in (TABLE pool_id_output)), -- ############# BLOCKCHAIN ################## !!
    ...
    cumulative_balance AS (
        ...
        AND t.blockchain = 'arbitrum' -- ############# BLOCKCHAIN ################## !!
    ),
