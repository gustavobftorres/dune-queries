...
-- query name: Ethereum ECLPs
-- query link: https://dune.com/queries/3363818
...
WITH pools AS (
  ...
  SELECT
    ...
    'ECLP' AS pool_type
  FROM balancer_v2_ethereum.Vault_evt_PoolRegistered c
  INNER JOIN gyroscope_ethereum.GyroECLPPoolFactory_call_create cc
    ON c.evt_tx_hash = cc.call_tx_hash
    AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
  CROSS JOIN UNNEST(cc.tokens) AS t(tokens)
...
SELECT 
  ...
  END AS name,
  pool_type,
  r.evt_block_time AS creation_date,
...
LEFT JOIN balancer_v2_ethereum.Vault_evt_PoolRegistered r ON bytearray_substring(s.pool_id, 1, 20) = r.poolAddress
WHERE pool_type = 'ECLP'
GROUP BY pool_id, pool_symbol, pool_type, evt_block_time
ORDER BY 1
