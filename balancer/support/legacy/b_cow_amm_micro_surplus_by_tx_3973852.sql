...
-- query name: b_cow_amm_micro_surplus_by_tx
-- query link: https://dune.com/queries/3973852
...
with t1 AS(
    ...
    JOIN balancer_cowswap_amm.trades c ON d.tx_hash = c.tx_hash AND c.blockchain = d.blockchain
    WHERE istrade AND cow_amm_address = {{1. Pool Address}} AND token_1_transfer*token_2_transfer < 0
    ...
    FROM dune.balancer.result_b_cow_amm_base_table d
    JOIN erc20_{{4. Blockchain}}.evt_transfer e ON d.tx_hash = e.evt_tx_hash
    WHERE istrade AND cow_amm_address = {{1. Pool Address}} AND token_1_transfer*token_2_transfer < 0 
    AND d.blockchain = '{{4. Blockchain}}'
    ),
