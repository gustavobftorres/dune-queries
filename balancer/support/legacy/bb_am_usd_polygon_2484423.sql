...
-- query name: bb-am-USD (POLYGON)
-- query link: https://dune.com/queries/2484423
...
-----------------------------------------------------------------------------------
    ...
    pool_id_output AS (
        WITH 
            input AS (SELECT array[0xb59be8f3c85a9dd6e2899103b6fbf6ea405b99a4000000000000000000000b34, 0x89b28a9494589b09dbccb69911c189f74fdadc5a000000000000000000000b33, 0xa5fe91dde37d8bf2dacacc0168b115d28ed03f84000000000000000000000b35] AS pool_ids)
...
-----------------------------------------------------------------------------------
 ...
 -----------------------------------------------------------------------------------  
    ...
    pool_params AS (SELECT * FROM query_2407001 WHERE blockchain = 'polygon' AND linear_pool in (TABLE linear_pool_addresses)), -- ############# BLOCKCHAIN ################## !!
    -- Create a data subset where we get swaps from the Vault filtered by the inputted pool id(s)
    subset as (SELECT evt_block_time, poolId, tokenIn, tokenOut, amountIn, amountOut FROM balancer_v2_polygon.Vault_evt_Swap WHERE poolId in (TABLE pool_id_output)), -- ############# BLOCKCHAIN ################## !!
    -- Create a data subset where we get Balance changes by the pool manager from the Vault filtered by the inputted pool id(s)
    subset_2 as (SELECT evt_block_time, poolId, token, cashDelta, managedDelta FROM balancer_v2_polygon.Vault_evt_PoolBalanceManaged WHERE poolId in (TABLE pool_id_output)), -- ############# BLOCKCHAIN ################## !!
    ...
    cumulative_balance AS (
        ...
        AND t.blockchain = 'polygon' -- ############# BLOCKCHAIN ################## !!
    ),
