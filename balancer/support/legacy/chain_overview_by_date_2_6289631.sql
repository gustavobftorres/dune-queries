...
-- query name: Chain overview by date (2)
-- query link: https://dune.com/queries/6289631
...
WITH swaps AS
    ...
    FROM dex.trades
    WHERE blockchain IN ('polygon', 'ethereum', 'arbitrum', 'gnosis', 'optimism','base','avalanche_c', 'zkevm', 'plasma') AND block_time >= CAST(DATE('{{target_date_2}}') AS TIMESTAMP) - INTERVAL '1' DAY
    GROUP BY 1),
    ...
    revenue AS(
        ...
        WHERE day > DATE('{{target_date_2}}') - interval '30' day
        GROUP BY 1
    )
