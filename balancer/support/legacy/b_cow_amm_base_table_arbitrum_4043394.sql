...
-- query name: b_cow_amm_base_table_arbitrum
-- query link: https://dune.com/queries/4043394


with
  arbitrum_cow_amms_temp as (
    SELECT 
        a.contract_address AS cow_amm_address,
        a.token AS token_1_address,
        b.token AS token_2_address
    FROM b_cow_amm_arbitrum.BCoWPool_call_bind a
    JOIN b_cow_amm_arbitrum.BCoWPool_call_bind b
    ON a.contract_address = b.contract_address
    WHERE a.token < b.token
    AND a.call_success),
  
 arbitrum_cow_amms as (
    select
...
transfers_temp as (
      SELECT 'arbitrum' as blockchain, contract_address as token_address,
        (CASE WHEN "from" IN (SELECT cow_amm_address from arbitrum_cow_amms_temp) then "from" else to end) as cow_amm_address,
        (CASE WHEN "from" IN (SELECT cow_amm_address from arbitrum_cow_amms_temp) then -value else value end) as net_value,
        evt_block_time as time, evt_tx_hash as tx_hash
      FROM erc20_arbitrum.evt_transfer
      WHERE ("from" IN (SELECT cow_amm_address from arbitrum_cow_amms)
        or "to" IN (SELECT cow_amm_address from arbitrum_cow_amms))
        AND evt_block_time > TIMESTAMP '2024-07-01'
        
  --add also the deposit events for WETH
  UNION
      SELECT 'arbitrum' as blockchain, 0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2 as token_address,
      varbinary_substring(topic1,13,20) as cow_amm_address, varbinary_to_uint256(data) as net_value,
      block_time as time, tx_hash
      from arbitrum.logs
      where varbinary_substring(topic1,13,20) in (select cow_amm_address from arbitrum_cow_amms)
  ...
  trades as(
        select 'arbitrum' as blockchain, t.tx_hash, t.order_uid, t.trader as cow_amm_address,
        ...
        where trader in (select cow_amm_address from arbitrum_cow_amms)
    ),
   ...
   cow_amms_evolution_temp as(
    ...
    cow_amms_evolution as (
        ...
        left join (select distinct minute, price, symbol, decimals from prices.usd where blockchain = 'arbitrum') p11 on token_1_symbol = p11.symbol  and date_trunc('minute',t.time) = p11.minute 
        left join (select distinct minute, price, symbol, decimals from prices.usd where blockchain = 'arbitrum') p21 on token_2_symbol = p21.symbol and date_trunc('minute',t.time) = p21.minute
    )
