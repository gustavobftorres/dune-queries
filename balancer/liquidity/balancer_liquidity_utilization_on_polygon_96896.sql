...
-- query name: Balancer Liquidity Utilization on Polygon
-- query link: https://dune.com/queries/96896
...
WITH prices AS (
    ...
    volume AS (
        ...
        AND ('{{1. Pool ID}}' = 'All' OR
        exchange_contract_address = CONCAT('\', SUBSTRING('{{1. Pool ID}}', 2))::bytea)
        GROUP BY 1
    ),
