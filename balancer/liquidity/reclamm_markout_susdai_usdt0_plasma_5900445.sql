...
-- query name: Reclamm Markout sUSDai/USDT0 - Plasma
-- query link: https://dune.com/queries/5900445
...
price_chain_A AS (
    SELECT date_trunc('minute', "timestamp" - interval '5' minute) as "minute", CAST(price as DOUBLE) as price
    FROM prices.minute
    WHERE 
        blockchain = 'arbitrum' 
        and contract_address = 0x0b2b2b2076d95dda7817e785989fe353fe955ef9
        and "timestamp" > TIMESTAMP '2025-09-23 00:00:00'
        and "timestamp" < now()
),
price_chain_B AS (
...
price_reclamm AS (
    ...
    WHERE S.pool = 0xb3ca3ead1c59ded552cd30a6992038284b418b65 AND S.evt_block_time > TIMESTAMP '2025-09-23 00:00:00' AND S.evt_block_time < now()
),
