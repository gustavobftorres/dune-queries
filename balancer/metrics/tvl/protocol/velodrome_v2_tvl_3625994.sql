...
-- query name: Velodrome V2 TVL
-- query link: https://dune.com/queries/3625994
...
pool_data AS (
  ...
  FROM velodrome_v2_optimism.PoolFactory_evt_PoolCreated AS p
  LEFT JOIN tokens.erc20 AS t1
    ON t1.contract_address = token0 AND t1.blockchain = 'optimism'
  LEFT JOIN tokens.erc20 AS t2
    ON t2.contract_address = token1 AND t2.blockchain = 'optimism'
), 
...
daily_balances AS (
  ...
  FROM velodrome_v2_optimism.Pool_evt_Sync AS d
...
LEFT JOIN query_3629980 m ON t.pool = m.pool AND m.project = 'velodrome'
