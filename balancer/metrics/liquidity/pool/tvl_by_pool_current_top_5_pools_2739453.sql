...
-- query name: TVL by Pool, Current Top 5 Pools
-- query link: https://dune.com/queries/2739453
...
tvl AS (
    ...
    WHERE
        ...
        AND ('{{1. Pool ID}}' = 'All' OR SUBSTRING(CAST(pool_id AS VARCHAR), 1, 42) = SUBSTRING('{{1. Pool ID}}', 1, 42))
    GROUP BY 1, 2, 3, 4
),
