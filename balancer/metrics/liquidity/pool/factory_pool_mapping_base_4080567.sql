...
-- query name: Factory <> Pool Mapping (base)
-- query link: https://dune.com/queries/4080567
...
WITH pools AS (
  ...
  FROM (
    ...
    INNER JOIN balancer_v2_base.WeightedPoolFactory_call_create cc
  ...
  SELECT
    ...
    'stable' AS pool_type
  FROM balancer_v2_base.Vault_evt_PoolRegistered c
  INNER JOIN balancer_v2_base.ComposableStablePoolFactory_call_create cc
    ON c.evt_tx_hash = cc.call_tx_hash
    AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
  CROSS JOIN UNNEST(cc.tokens) AS t(tokens)
  ...
  SELECT
    ...
    'linear' AS pool_type
  FROM balancer_v2_base.Vault_evt_PoolRegistered c
  INNER JOIN balancer_v2_base.AaveLinearPoolFactory_call_create cc
    ON c.evt_tx_hash = cc.call_tx_hash
    AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
  ...
  SELECT
    ...
    'linear' AS pool_type
  FROM balancer_v2_base.Vault_evt_PoolRegistered c
  INNER JOIN balancer_v2_base.ERC4626LinearPoolFactory_call_create cc
    ON c.evt_tx_hash = cc.call_tx_hash
    AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
  CROSS JOIN UNNEST(ARRAY[cc.mainToken, cc.wrappedToken]) AS t (element)
  ...
  FROM balancer_v2_base.Vault_evt_PoolRegistered c
  INNER JOIN balancer_v2_base.ManagedPoolFactory_call_create cc
    ON c.evt_tx_hash = cc.call_tx_hash
  ...
  SELECT
    ...
    'ECLP' AS pool_type
  FROM balancer_v2_base.Vault_evt_PoolRegistered c
  INNER JOIN gyroscope_base.GyroECLPPoolFactory_call_create cc
    ON c.evt_tx_hash = cc.call_tx_hash
    AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
  CROSS JOIN UNNEST(cc.tokens) AS t(tokens)    
),
