...
-- query name: BPT supply spell
-- query link: https://dune.com/queries/3340148
...
WITH pool_labels AS (
  ...
  -- Extract mints and burns from transfers
    ...
    premints_1 AS (
        ...
        FROM balancer_v2_ethereum.Vault_evt_PoolBalanceChanged pb
   ...
   -- SELECT * FROM joins_and_exits WHERE bpt = 0x5aee1e99fe86960377de9f88689616916d5dcabe
   ...
   SELECT
        ...
        SUM(b.supply + COALESCE(adelta, 0)) AS totalSupply,
