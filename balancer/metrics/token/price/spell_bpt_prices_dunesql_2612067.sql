...
-- query name: spell_bpt_prices_dunesql
-- query link: https://dune.com/queries/2612067
...
WITH
    ...
    price_formulation AS (
        SELECT
            ...
            t.symbol AS symbol,
            approx_percentile(b.price, 0.5) AS median_price
        FROM (
        ...
        LEFT JOIN tokens.erc20 t ON t.contract_address = b.contract_address AND t.blockchain = 'ethereum'
        GROUP BY 1, 2, 3, 4, 5
        ORDER BY 2 DESC, 3
    )
