...
-- query name: bpt_prices_spell
-- query link: https://dune.com/queries/2508419


WITH
    bpt_trades AS (
        SELECT evt_tx_hash FROM balancer_v2_polygon.Vault_evt_Swap t
        WHERE ( CAST(t.tokenIn AS VARCHAR(66)) = SUBSTRING(CAST(t.poolId AS VARCHAR(66)), 1, 42)
        OR CAST(t.tokenOut AS VARCHAR(66)) = SUBSTRING(CAST(t.poolId AS VARCHAR(66)), 1, 42) ) 
    ),
    
    all_trades_given_bpt_tx AS (SELECT * FROM balancer_v2_polygon.Vault_evt_Swap WHERE evt_tx_hash IN (TABLE bpt_trades)), 
    ...
    all_trades_info AS (
        ...
        LEFT JOIN prices.usd p1 ON p1.contract_address = a.tokenIn AND p1.blockchain = 'polygon' AND  p1.minute = date_trunc('minute', a.evt_block_time)
        LEFT JOIN prices.usd p2 ON p2.contract_address = a.tokenOut AND p2.blockchain = 'polygon' AND  p2.minute = date_trunc('minute', a.evt_block_time)
        LEFT JOIN tokens.erc20 t1 ON t1.contract_address = a.tokenIn AND t1.blockchain = 'polygon'
        LEFT JOIN tokens.erc20 t2 ON t2.contract_address = a.tokenOut AND t2.blockchain = 'polygon'
