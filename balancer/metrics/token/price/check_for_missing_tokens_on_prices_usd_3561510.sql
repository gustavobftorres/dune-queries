...
-- query name: Check for missing tokens on prices.usd
-- query link: https://dune.com/queries/3561510
...
-- liquidity formulation, with a few simplifications, compared to liquidity spell
    ...
    swaps_changes AS (
        ...
        FROM
            (
                ...
                FROM balancer_v2_zkevm.Vault_evt_Swap
    ...
    zipped_balance_changes AS (
        ...
        FROM balancer_v2_zkevm.Vault_evt_PoolBalanceChanged
    ...
    managed_changes AS (
        ...
        FROM balancer_v2_zkevm.Vault_evt_PoolBalanceManaged
    ),
   ...
   cumulative_usd_balance AS (
    ...
    SELECT t.pool_address, l.pool_symbol, COALESCE(liquidity, 0) AS price, sum(protocol_liquidity_usd) AS spell
    FROM tvl t
    LEFT JOIN balancer_v2_zkevm.liquidity l ON t.pool_address = l.pool_address
    AND l.day = t.day
