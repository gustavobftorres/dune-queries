...
-- query name: Balancer Pool LPs Distribution on Polygon
-- query link: https://dune.com/queries/20914


WITH joins AS (
        SELECT date_trunc('day', e.evt_block_time) AS day, e."to" AS lp, e.contract_address AS pool, SUM(e.value)/1e18 AS amount
    ...
    exits AS (
        SELECT date_trunc('day', e.evt_block_time) AS day, e."from" AS lp, e.contract_address AS pool, -SUM(e.value)/1e18 AS amount
        FROM balancer_v2."WeightedPool_evt_Transfer" e
