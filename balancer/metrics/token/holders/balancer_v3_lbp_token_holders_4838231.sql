...
-- query name: Balancer V3 LBP Token Holders
-- query link: https://dune.com/queries/4838231
...
WITH lbp_info AS (
    ...
    addresses AS (
        ...
        INNER JOIN lbp_info l ON l.project_token = tr.contract_address AND tr.blockchain = l.blockchain
    ), 
    ...
    transfers AS (
        ...
        FROM (
            ...
            INNER JOIN lbp_info l ON l.project_token = tr.contract_address
...
SELECT  CAST(b.day as timestamp) as day, l.project_token_symbol AS token_symbol, COUNT(DISTINCT address) AS holders
FROM balance_all_days b
INNER JOIN lbp_info l ON l.project_token = b.token_address
