...
-- query name: crashed_tokens_lending
-- query link: https://dune.com/queries/6951733
...
WITH borrow_contracts AS (
    SELECT address
    FROM bnb.contracts
    WHERE bytearray_length(code) BETWEEN 5000 AND 50000
...
SELECT 
    tr.token,
    tr.recipient as contract_address,
    tr.transfer_count
FROM token_recipients tr
JOIN borrow_contracts bc ON LOWER(CAST(bc.address AS VARCHAR)) = LOWER(tr.recipient)
ORDER BY tr.transfer_count DESC
LIMIT 200
