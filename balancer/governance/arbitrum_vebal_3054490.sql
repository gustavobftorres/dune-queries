...
-- query name: Arbitrum veBAL
-- query link: https://dune.com/queries/3054490
...
, final AS (
    ...
    FROM
    	vote_results AS v
    LEFT JOIN total_votes t ON t.end_date = v.end_date
...
, swaps as (
    SELECT
        ...
        , SUM(volume) OVER (ORDER BY week ASC ROWS BETWEEN 2 PRECEDING AND CURRENT ROW ) / 3 AS vol_sma_3
        , SUM(volume) OVER (ORDER BY week ASC ROWS BETWEEN 4 PRECEDING AND CURRENT ROW ) / 5 AS vol_sma_5
    FROM (
        SELECT
...
SELECT --*
    ...
    , s.vol_sma_3
    , s.vol_sma_5
    , SUM(pct_votes) OVER (ORDER BY end_date ASC ROWS BETWEEN 2 PRECEDING AND CURRENT ROW ) / 3 AS pct_votes_sma_3
	, SUM(pct_votes) OVER (ORDER BY end_date ASC ROWS BETWEEN 4 PRECEDING AND CURRENT ROW ) / 5 AS pct_votes_sma_5
FROM final x
...
INNER JOIN (
    ...
    ON z.day = x.end_date
INNER JOIN swaps s
    ON (week(s.week) = week(x.end_date) AND year(s.week) = year(x.end_date))
