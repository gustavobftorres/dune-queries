...
-- query name: Balancer Growth
-- query link: https://dune.com/queries/20915
...
WITH prices AS (
    ...
    dex_prices AS (
        select date_trunc('day', hour) AS day, contract_address AS token, APPROX_PERCENTILE(median_price,0.5) AS price
        FROM dex.prices
    ...
    pool_liquidity_estimates AS (
        ...
        balancer_v1_ethereum.pools_tokens_weights w
        ON b.pool = BYTEARRAY_SUBSTRING(w.pool_id,1,20)
    ...
    TVL_at_end_of_sunday AS (
        ...
        date_trunc('week', day) + INTERVAL '6' day as sunday, 
        AVG(amount) as amount
        FROM total_value_locked
        GROUP BY 1
     --   WHERE extract(dow from date_trunc('week', day) + INTERVAL '6' day) = 0 
    ...
    daily_delta_usd_balance_by_token AS ( 
        SELECT b.day, b.token, COALESCE(amount /POWER(10, t.decimals) * p1.price, amount /POWER(10, t.decimals) * p2.price, 0) AS amount
...
select 
  CAST(initial_tvl_and_growth.monday_of_the_week AS TIMESTAMP) as week, 
  initial_tvl_and_growth.growth * 100 as "Growth (%)",
  tvl_after_growth.initial_tvl/POWER(10, 6) as "TVL (million USD)"
from growth initial_tvl_and_growth
inner join growth tvl_after_growth
on initial_tvl_and_growth.monday_of_the_week = tvl_after_growth.monday_of_the_week - INTERVAL '7' day
WHERE tvl_after_growth.initial_tvl < 1e9
    AND initial_tvl_and_growth.growth < 1e3
    AND initial_tvl_and_growth.growth > -1e4
    AND CAST(initial_tvl_and_growth.monday_of_the_week AS TIMESTAMP) > TIMESTAMP '2020-04-01'