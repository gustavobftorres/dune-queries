...
-- query name: wip
-- query link: https://dune.com/queries/2441394


WITH
    -- Get the list of tx_hash's where the tokenIn or tokenOut of the Balancer Vault is the BPT
    bpt_trades AS (
        SELECT * FROM balancer_v2_ethereum.Vault_evt_Swap v
    ...
    -- Using the list of tx_hash's get all the other Balancer Vault Swaps that occur in the tx_hash.
    -- This information will be used to help price the BPT's when they are traded against one another e.g. ComposableStablePools.
    --all_trades_given_bpt_tx AS (SELECT * FROM balancer_v2_ethereum.Vault_evt_Swap WHERE evt_tx_hash IN (TABLE bpt_trades)), 
    
    -- Get the pricing, decimal, and symbol for tokenIn and tokenOut.
    -- Additionally, change some data types and create the bpt_address.     
    all_trades_info AS (
        SELECT 
        ...
        FROM bpt_trades a
        LEFT JOIN prices.usd p1 ON p1.contract_address = a.tokenIn AND p1.blockchain = 'ethereum' AND  p1.minute = date_trunc('minute', a.evt_block_time)
        LEFT JOIN prices.usd p2 ON p2.contract_address = a.tokenOut AND p2.blockchain = 'ethereum' AND  p2.minute = date_trunc('minute', a.evt_block_time)
