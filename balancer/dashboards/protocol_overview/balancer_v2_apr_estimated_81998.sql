...
-- query name: Balancer V2 APR Estimated
-- query link: https://dune.com/queries/81998
...
WITH prices AS (
    ...
    weekly_revenues AS (
        SELECT date_trunc('day', day) AS day, SUM(revenues) AS revenues
        FROM revenues
        GROUP BY 1
    ),
    ...
    total_tvl AS (
        SELECT date_trunc('day', day) AS day, SUM(liquidity) AS tvl
