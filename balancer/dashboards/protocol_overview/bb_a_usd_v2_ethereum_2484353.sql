...
-- query name: bb-a-USD v2 (ETHEREUM)
-- query link: https://dune.com/queries/2484353
    ...
    pool_id_output AS (
        WITH 
            input AS (SELECT array[
                0x82698aecc9e28e9bb27608bd52cf57f704bd1b83000000000000000000000336, 0xae37d54ae477268b9997d4161b96b8200755935c000000000000000000000337, 0x2f4eb100552ef93840d5adc30560e5513dfffacb000000000000000000000334
                ] AS pool_ids)
...
-----------------------------------------------------------------------------------
 ...
 -----------------------------------------------------------------------------------  
    ...
    pool_params AS (SELECT * FROM query_2407001 WHERE blockchain = 'ethereum' AND linear_pool in (TABLE linear_pool_addresses)), -- ############# BLOCKCHAIN ################## !!
    -- Create a data subset where we get swaps from the Vault filtered by the inputted pool id(s)
    subset as (SELECT evt_block_time, poolId, tokenIn, tokenOut, amountIn, amountOut FROM balancer_v2_ethereum.Vault_evt_Swap WHERE poolId in (TABLE pool_id_output)), -- ############# BLOCKCHAIN ################## !!
    -- Create a data subset where we get Balance changes by the pool manager from the Vault filtered by the inputted pool id(s)
    subset_2 as (SELECT evt_block_time, poolId, token, cashDelta, managedDelta FROM balancer_v2_ethereum.Vault_evt_PoolBalanceManaged WHERE poolId in (TABLE pool_id_output)), -- ############# BLOCKCHAIN ################## !!
    ...
    cumulative_balance AS (
        ...
        AND t.blockchain = 'ethereum' -- ############# BLOCKCHAIN ################## !!
    ),
