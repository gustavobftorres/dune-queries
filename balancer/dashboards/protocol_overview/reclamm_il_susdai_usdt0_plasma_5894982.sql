...
-- query name: reCLAMM IL sUSDai/USDT0 - Plasma
-- query link: https://dune.com/queries/5894982
...
WITH price_token_a_hourly AS (
    ...
    WHERE 
        ...
        AND "timestamp" > TIMESTAMP '2025-09-27 00:00:00'
...
price_token_b_hourly AS (
    ...
    WHERE 
        ...
        AND minute > TIMESTAMP '2025-09-27 00:00:00'
        AND minute("minute") = 0
),
bpt_supply_hourly as (
    SELECT hour, total_supply FROM "query_5874871(pool='0xb3ca3ead1c59ded552cd30a6992038284b418b65')"
),
pool_liquidity_hourly as (
    SELECT hour, token_a_balance, token_b_balance FROM "query_5875552(pool='0xb3ca3ead1c59ded552cd30a6992038284b418b65')"
),
bpt_prices as (
    SELECT L.hour, PA.price*L.token_a_balance + PB.price*token_b_balance as pool_tvl, 100000*(PA.price*L.token_a_balance + PB.price*token_b_balance)/BS.total_supply as bpt_price, PA.price as price_a, PB.price as price_b, token_a_balance, token_b_balance, total_supply
    ...
    WHERE L.hour > TIMESTAMP '2025-09-27 00:00'
),
