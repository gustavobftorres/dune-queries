...
-- query name: BAL Emissions and Fees Collected, per Round of Emissions
-- query link: https://dune.com/queries/3663975
...
SELECT 
    ...
    start_date,
    end_date,
    SUM(SUM(daily_emissions)) OVER (PARTITION BY round_id ORDER BY round_id) AS round_emissions,
    SUM(SUM(daily_emissions_usd)) OVER (PARTITION BY round_id ORDER BY round_id) AS round_emissions_usd,
    SUM(SUM(daily_fees)) OVER (PARTITION BY round_id ORDER BY round_id) AS round_fees,
    SUM(SUM(daily_fees)) OVER (PARTITION BY round_id ORDER BY round_id) / SUM(SUM(daily_emissions_usd)) OVER (PARTITION BY round_id ORDER BY round_id) AS ratio
