...
-- query name: Balancer Volume by Source - DEX comparison
-- query link: https://dune.com/queries/3632600
...
WITH 
    raw_swaps AS (
        ...
        LEFT JOIN labels.balancer_v2_pools l ON l.address = d.project_contract_address AND l.blockchain = d.blockchain
        WHERE block_time >= TIMESTAMP '{{Start Date}}'
        AND ('{{Balancer Blockchain}}' = 'All' OR d.blockchain = '{{Balancer Blockchain}}')
