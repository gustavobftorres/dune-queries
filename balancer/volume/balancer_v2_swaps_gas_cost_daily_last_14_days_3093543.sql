...
-- query name: Balancer V2 Swaps Gas Cost, daily (Last 14 days)
-- query link: https://dune.com/queries/3093543
...
WITH
  txns as(
    ...
    WHERE tx.block_time > (CURRENT_DATE - INTERVAL '14' day) 
    AND s.evt_block_time > (CURRENT_DATE - INTERVAL '14' day)
...
FROM
 ...
 WHERE n_swaps = 1
 GROUP BY 1,2,3
 ORDER BY 1 DESC, 8 DESC --order by day and median gas_per_swap