...
-- query name: Balancer 7-day Volume (Heavy Trader Breakdown) on Polygon
-- query link: https://dune.com/queries/141923
...
WITH prices AS (
    ...
    swaps AS (
        ...
        AND block_time > date_trunc('day', now() - interval '7d')
    ...
    heavy_traders AS (
        SELECT
            channel, day, txns AS daily_trades
        FROM swaps
        WHERE txns >= 100
    ),
