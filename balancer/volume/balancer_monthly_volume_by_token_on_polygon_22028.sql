...
-- query name: Balancer Monthly Volume by Token on Polygon
-- query link: https://dune.com/queries/22028
...
WITH prices AS (
    ...
    ranking AS (
        ...
        FROM swaps
        WHERE month = date_trunc('month', CURRENT_DATE - interval '1 week')
        GROUP BY 1
    ),
    
    total_volume AS (
        SELECT month, 'Total' AS token, SUM(volume) AS volume
        FROM swaps
        GROUP BY 1
    )
