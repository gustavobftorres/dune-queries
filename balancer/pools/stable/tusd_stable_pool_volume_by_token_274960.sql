-- part of a query repo
-- query name: TUSD Stable Pool - Volume by Token
-- query link: https://dune.com/queries/274960


-- Volume (token breakdown) per week (3 months)
-- Visualization: bar chart (stacked)

WITH swaps AS (
        SELECT
            date_trunc('week', d.block_time) AS week,
            sum(usd_amount) AS volume,
            d.token_b_address AS address,
            t.symbol AS token
        FROM dex.trades d
        LEFT JOIN erc20.tokens t ON t.contract_address = d.token_b_address
        WHERE project = 'Balancer'
        AND exchange_contract_address = '\x06df3b2bbb68adc8b0e302443692037ed9f91b42000000000000000000000012'
        GROUP BY 1,3,4
        
        UNION ALL
        
        SELECT
            date_trunc('week', d.block_time) AS week,
            sum(usd_amount) AS volume,
            d.token_a_address AS address,
            t.symbol AS token
        FROM dex.trades d
        LEFT JOIN erc20.tokens t ON t.contract_address = d.token_a_address
        WHERE project = 'Balancer'
        AND exchange_contract_address = '\x06df3b2bbb68adc8b0e302443692037ed9f91b42000000000000000000000012'
        GROUP BY 1,3,4
)

SELECT
    s.week,
    COALESCE(s.token, CONCAT(SUBSTRING(s.address::text, 3, 6), '...')) AS token,
    s.address,
    sum(s.volume)/2 AS volume
FROM swaps s
WHERE week >= '2021-11-15'
GROUP BY 1, 2, 3
ORDER BY 1, 3