...
-- query name: bcowamm 80/20 pools
-- query link: https://dune.com/queries/4109250
...
WITH events AS (
    ...
    SELECT call_block_number AS block_number,
           ...
           token,
           denorm
    FROM balancer_testnet_sepolia.BCoWPool_call_bind
    WHERE call_success
    ...
    SELECT call_block_number AS block_number,
            ...
            token,
            uint256 '0' AS denorm
    FROM balancer_testnet_sepolia.BCoWPool_call_unbind
    WHERE call_success
),
