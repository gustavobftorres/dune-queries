...
-- query name: Monthly Yield Fees Cut + Core Pools Study 2 (Last 3m)
-- query link: https://dune.com/queries/4223635
...
WITH fees AS (
    SELECT 
        ...
        blockchain,
        SUM(total_protocol_yield_fee)/3 AS yield_fee,
        SUM(total_protocol_swap_fee)/3 AS swap_fee
    ...
    AND block_month <= TIMESTAMP '2026-01-31'
...
SELECT
    ...
    SUM(swap_fee) AS actual_protocol_swap_fee,
    SUM(yield_fee) AS actual_lp_yield_fee,
    SUM(swap_fee) AS actual_lp_swap_fee,
    SUM(yield_fee * 0.175) AS actual_dao_yield_fee,
    SUM(swap_fee * 0.175) AS actual_dao_swap_fee,
    SUM(yield_fee * 0.325 ) AS actual_vebal_yield_fee,
    SUM(swap_fee * 0.325) AS actual_vebal_swap_fee,
