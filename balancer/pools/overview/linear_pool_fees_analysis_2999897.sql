...
-- query name: Linear Pool Fees Analysis
-- query link: https://dune.com/queries/2999897


WITH
    sub_pools AS (
        -- Return all the info of the subpools
        SELECT
            contract_address
            , evt_tx_hash
            , evt_index
            ...
            , CASE -- Hard code an ordered token array structure for future use
                WHEN linear_pool_token = tokens[1] THEN array[tokens[1], tokens[2], tokens[3]]
                WHEN linear_pool_token = tokens[2] THEN array[tokens[2], tokens[1], tokens[3]]
                WHEN linear_pool_token = tokens[3] THEN array[tokens[3], tokens[1], tokens[2]]
            END AS tokens
        FROM balancer_v2_{{blockchain}}.Vault_evt_TokensRegistered x
        INNER JOIN (
            -- Unnest token array of tokens in the CSP
            SELECT t.csp_tokens AS linear_pool_token FROM (
                -- Get the info array of tokens in the inputed CSP
                SELECT tokens AS csp_tokens
                FROM balancer_v2_{{blockchain}}.Vault_evt_TokensRegistered
                WHERE poolId = {{csp_pool_id}}
            ) CROSS JOIN UNNEST(csp_tokens) AS t(csp_tokens)
        ) t 
            ON t.linear_pool_token = bytearray_substring(x.poolId, 1, 20)
        WHERE {{csp_pool_id}} <> x.poolId
    )
    ...
    , join_exit_call_data AS ( 
        ...
        FROM ( --Start subquery
            SELECT 
                ...
                , x.evt_block_number
                , x.poolId AS poolId
                , sp.linear_pool_token AS pool_token
                ...
                , CAST(y.value AS int256) AS lp_token_burned
                , x.liquidityProvider
                , sp.tokens = x.tokens
                , sp.tokens
            FROM balancer_v2_{{blockchain}}.Vault_evt_PoolBalanceChanged x
            INNER JOIN (SELECT poolId, tokens, linear_pool_token FROM sub_pools) sp
            ...
            INNER JOIN (
                ...
                AND y.contract_address = sp.linear_pool_token
                AND y.to = 0x0000000000000000000000000000000000000000
        ) x -- End subquery
    ...
    -- When doing this for multiple tables they must be ordered identically.
        SELECT * 
            , ROW_NUMBER() OVER (
                ...
                ORDER BY t.bn_targets DESC, t.ai_targets DESC, f.bn_swapfee DESC, f.ai_swapfee DESC
            ) AS rn_targets
            , ROW_NUMBER() OVER (
                PARTITION BY x.pool_id, x.tx_hash, x.action_index 
                ORDER BY t.bn_targets DESC, t.ai_targets DESC, f.bn_swapfee DESC, f.ai_swapfee DESC
