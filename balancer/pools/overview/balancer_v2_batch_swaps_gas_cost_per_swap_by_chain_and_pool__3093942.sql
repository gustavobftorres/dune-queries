...
-- query name: Balancer V2 Batch Swaps Gas Cost per swap, by chain and pool type
-- query link: https://dune.com/queries/3093942
...
SELECT
    ...
    AS blockchain,
  g.n_swaps,
  APPROX_PERCENTILE(g.gas_per_swap, 0.01) AS p01_gas_per_swap,
  APPROX_PERCENTILE(g.gas_per_swap, 0.05) AS p05_gas_per_swap,
  APPROX_PERCENTILE(g.gas_per_swap, 0.1) AS p10_gas_per_swap,
  APPROX_PERCENTILE(g.gas_per_swap, 0.25) AS p25_gas_per_swap,
  APPROX_PERCENTILE(g.gas_per_swap, 0.5) AS p50_gas_per_swap,
  APPROX_PERCENTILE(g.gas_per_swap, 0.75) AS p75_gas_per_swap,
  APPROX_PERCENTILE(g.gas_per_swap, 0.90) AS p90_gas_per_swap
FROM
  gas_per_swap as g
 WHERE ('{{# of swaps}}' = 'Any' OR n_swaps = CAST('{{# of swaps}}' as BIGINT))
 AND ('{{blockchain}}' = 'All' or g.blockchain = '{{blockchain}}')
 GROUP BY 1,2
 ORDER BY --1, 2, 3 ASC, 8 DESC
2 ASC, 1, 7 DESC