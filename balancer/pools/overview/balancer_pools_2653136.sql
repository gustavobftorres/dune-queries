...
-- query link: https://dune.com/queries/2653136
...
WITH volume AS (
    ...
    SELECT
      ...
      q.blockchain || 
        ...
        END 
    AS blockchain
    ,CONCAT('<a target="_blank" href="https://dune.com/balancer/pool-analysis?1.+Pool+ID_t1b222=', SUBSTRING(CAST("poolID" AS VARCHAR), 1, 66), '&4.+Blockchain_t9819b=', q.blockchain, '">Stats ↗</a>') AS stats,
        CASE
            WHEN q.blockchain = 'ethereum' THEN CONCAT('<a target="_blank" href="https://app.balancer.fi/#/ethereum/pool/', CAST("poolID" AS VARCHAR), '">app ↗</a>')
            WHEN q.blockchain = 'arbitrum' THEN CONCAT('<a target="_blank" href="https://app.balancer.fi/#/arbitrum/pool/', CAST("poolID" AS VARCHAR), '">app ↗</a>')
            WHEN q.blockchain = 'polygon' THEN CONCAT('<a target="_blank" href="https://app.balancer.fi/#/polygon/pool/', CAST("poolID" AS VARCHAR), '">app ↗</a>')
            WHEN q.blockchain = 'gnosis' THEN CONCAT('<a target="_blank" href="https://app.balancer.fi/#/gnosis-chain/pool/', CAST("poolID" AS VARCHAR), '">app ↗</a>')
            WHEN q.blockchain = 'base' THEN CONCAT('<a target="_blank" href="https://app.balancer.fi/#/base/pool/', CAST("poolID" AS VARCHAR), '">app ↗</a>')
            WHEN q.blockchain = 'avalanche_c' THEN CONCAT('<a target="_blank" href="https://app.balancer.fi/#/avalanche/pool/', CAST("poolID" AS VARCHAR), '">app ↗</a>')
            WHEN q.blockchain = 'optimism' THEN CONCAT('<a target="_blank" href="https://beets.fi/pool/', CAST("poolID" AS VARCHAR), '">app ↗</a>')
            WHEN q.blockchain = 'zkevm' THEN CONCAT('<a target="_blank" href="https://app.balancer.fi/#/zkevm/pool/', CAST("poolID" AS VARCHAR), '">app ↗</a>')
        END AS pool,
        CASE
    ...
    WHERE
        ('{{3. Blockchain}}' = 'All' OR v.blockchain = '{{3. Blockchain}}')
        AND "TVL" IS NOT NULL
    ORDER BY 4 DESC
