...
-- query name: Factory <> Pool Mapping (gnosis)
-- query link: https://dune.com/queries/4080577
...
WITH pools AS (
  ...
  FROM (
    ...
    INNER JOIN balancer_v2_gnosis.WeightedPoolV2Factory_call_create cc
  ...
  FROM (
    SELECT
      ...
      'weighted' AS pool_type
    FROM balancer_v2_gnosis.Vault_evt_PoolRegistered c
    INNER JOIN balancer_v2_gnosis.WeightedPoolV4Factory_call_create cc
      ON c.evt_tx_hash = cc.call_tx_hash
      AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
    CROSS JOIN UNNEST(cc.tokens) WITH ORDINALITY t(tokens, pos)
    CROSS JOIN UNNEST(cc.normalizedWeights) WITH ORDINALITY w(weights, pos)
  ...
  cc.contract_address AS factory_address,
    ...
    'stable' AS pool_type
  FROM balancer_v2_gnosis.Vault_evt_PoolRegistered c
  INNER JOIN balancer_v2_gnosis.StablePoolV2Factory_call_create cc
    ON c.evt_tx_hash = cc.call_tx_hash
    AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
  ...
  cc.contract_address AS factory_address,
    ...
    'stable' AS pool_type
  FROM balancer_v2_gnosis.Vault_evt_PoolRegistered c
  INNER JOIN balancer_v2_gnosis.ComposableStablePoolFactory_call_create cc
    ON c.evt_tx_hash = cc.call_tx_hash
    AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
  CROSS JOIN UNNEST(cc.tokens) AS t(tokens)
  ...
  cc.contract_address AS factory_address,
    ...
    'linear' AS pool_type
  FROM balancer_v2_gnosis.Vault_evt_PoolRegistered c
  INNER JOIN balancer_v2_gnosis.AaveLinearPoolFactory_call_create cc
    ON c.evt_tx_hash = cc.call_tx_hash
    AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
  ...
  cc.contract_address AS factory_address,
    ...
    'linear' AS pool_type
  FROM balancer_v2_gnosis.Vault_evt_PoolRegistered c
  INNER JOIN balancer_v2_gnosis.AaveLinearPoolV3Factory_call_create cc
    ON c.evt_tx_hash = cc.call_tx_hash
    AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
  CROSS JOIN UNNEST(ARRAY[cc.mainToken, cc.wrappedToken]) AS t (element)
  ...
  cc.contract_address AS factory_address,
    ...
    'stable' AS pool_type
  FROM balancer_v2_gnosis.Vault_evt_PoolRegistered c
  INNER JOIN balancer_v2_gnosis.ComposableStablePoolV2Factory_call_create cc
    ON c.evt_tx_hash = cc.call_tx_hash
    AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
  CROSS JOIN UNNEST(cc.tokens) AS t(tokens)
  ...
  FROM balancer_v2_gnosis.Vault_evt_PoolRegistered c
  INNER JOIN balancer_v2_gnosis.ManagedPoolFactory_call_create cc
    ON c.evt_tx_hash = cc.call_tx_hash
  ...
  cc.contract_address AS factory_address,
    ...
    'ECLP' AS pool_type
  FROM balancer_v2_gnosis.Vault_evt_PoolRegistered c
  INNER JOIN gyroscope_gnosis.GyroECLPPoolFactory_call_create cc
    ON c.evt_tx_hash = cc.call_tx_hash
    AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
  CROSS JOIN UNNEST(cc.tokens) AS t(tokens)
),
