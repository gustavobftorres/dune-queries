...
-- query name: Factory <> Pool Mapping (optimism)
-- query link: https://dune.com/queries/4080584
...
WITH pools AS (
  ...
  SELECT
    ...
    'ECLP' AS pool_type
  FROM balancer_v2_optimism.Vault_evt_PoolRegistered c
  INNER JOIN gyroscope_optimism.GyroECLPPoolFactory_call_create cc
    ON c.evt_tx_hash = cc.call_tx_hash
    AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
  CROSS JOIN UNNEST(cc.tokens) AS t(tokens)),
