...
-- query name: Balancer Pools on Polygon
-- query link: https://dune.com/queries/96884
...
WITH prices AS (
    ...
    cumulative_usd_balance AS (
        ...
        (p.price * cumulative_amount / 10 ^ p.decimals) AS amount_usd_from_api,
        0 AS amount_usd_from_dex
        FROM calendar c
        LEFT JOIN cumulative_balance b ON b.day <= c.day AND c.day < b.day_of_next_change
        LEFT JOIN prices p ON p.day = c.day AND p.token = b.token
    ),
    ...
    last_tvl AS(
        ...
        FROM total_tvl t
        WHERE day = CURRENT_DATE - interval '1d'
    ),
